...
local shellcheck_disable_regex = vim.regex(
  [[^\s*#\s*shellcheck\s\+disable=\(\(SC\)\?\d\+\)\([,-]\(SC\)\?\d\+\)*\s*$]]
)
...
local search_region = function(
  ...
  local lines =
    vim.api.nvim_buf_get_lines(bufnr, region_start, region_end, false)
  for i = idx_start, idx_end, step do
    ...
    if continue_regex and not continue_regex:match_str(line) then return end
  end
end
...
local get_first_non_comment_row = function(bufnr, row)
  ...
  local _, match_row = search_region(
    bufnr,
    blank_or_comment_line_regex,
    first_non_shebang_row,
    row,
    nil,
    true
  )
...
local find_disable_directive = function(bufnr, row_start, row_end)
  local line, row = search_region(
    bufnr,
    shellcheck_disable_regex,
    row_start,
    row_end,
    blank_or_comment_line_regex
  )
...
local get_line_directive = function(bufnr, row)
  ...
  local row_end = file_directive and file_directive.row
    or get_first_non_shebang_row(bufnr)
  return find_disable_directive(bufnr, row_start, row_end)
end

local disable_action = function(
  bufnr,
  existing_directive,
  default_line,
  code,
  ...
  return {
    ...
    action = function()
      return disable_action(
        bufnr,
        get_file_directive(bufnr),
        get_first_non_shebang_row(bufnr) + 1,
        code,
        ''
      )
    end,
...
local generate_line_disable_action = function(bufnr, row, code, indentation)
  ...
  row = match_row + 1
  indentation = vim.api
    .nvim_buf_get_lines(bufnr, row - 1, row, false)[1]
    :match('^%s+') or ''
  return {

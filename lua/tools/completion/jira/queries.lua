...
M.generate_candidates = function(kind, candidates, entries, callback)
        ...
        on_exit = function(self, exit_code)
          ...
          if exit_code == 0 then
            candidates[candidate_index].documentation.value = table.concat(
              require('vim.lsp.util').convert_input_to_markdown_lines(self:result()),
              '\n'
            )
          end

  ...
  vim.api.nvim_create_autocmd({ 'FileType' }, {
    ...
    callback = function()
      ...
      for _, lsp_server in ipairs(lsp_servers) do
        ...
        if server ~= nil and not registry.is_installed(server) then vim.api.nvim_command('MasonInstall ' .. server) end
      end
      ...
      if tools == nil then return end
      for tool, is_mason_tool in pairs(tools) do
        if is_mason_tool and not registry.is_installed(tool) then vim.api.nvim_command('MasonInstall ' .. tool) end
      end
    end,
  })
end
